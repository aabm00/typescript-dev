let apples = 5;
let speed: string = "fast";
let hasName: boolean = true;

let nothingMuch: null = null;
let nothing: undefined = undefined;

// built in objects
let now: Date = new Date();

// Array
let colors: string[] = ["red", "green", "blue"];
let myNumbers: number[] = [1, 2, 3];
let truths: boolean[] = [true, true, false];

// Classes
class Car {}
let car: Car = new Car();

// Object literal
let point: { x: number; y: number } = {
  x: 10,
  y: 20,
};

// Function
// En realidad estamos anotando una variable que tiene asignada
// una funcion (diciendo que esta variable es una función)
const logNumber: (i: number) => void = (i: number) => {
  console.log(i);
};

// Otra manera
const logNumber1 = (i: number): void => {
  console.log(i);
};

// When to use annotations
// 1) Function that returns the 'any' type
const json = '{"x": 10, "y": 20}';

const coordinates = JSON.parse(json); //  Devuelve 'any'
console.log(coordinates); // {x: 10, y: 20};

const coordinates1: { x: number; y: number } = JSON.parse(json);
console.log(coordinates1); // {x: 10, y: 20};

// 2) When we declare a variable on one line
// and initalizate it later
let words = ["red", "green", "blue"];
let foundWord;
let foundWord1: boolean;

for (let i = 0; i < words.length; i++) {
  if (words[i] === "green") {
    foundWord = true;
    foundWord1 = true;
  }
}

// 3) Variable whose type cannot be inferred correctly
let numbers = [-10, -1, 12];
let numberAboveZero = false;
let numberAboveZero1: boolean | number = false;

for (let i = 0; i < numbers.length; i++) {
  if (numbers[i] > 0) {
    numberAboveZero = numbers[i];
    numberAboveZero1 = numbers[i];
  }
}
